...
try {
    ...
    $stmt = $pdo->prepare(
        'UPDATE expenses SET description = ?, amount_cents = ?, paid_by_member_id = ? WHERE id = ? AND trip_id = ?'
    );
    $stmt->execute([$description, $amountCents, $paidByMemberId, $expenseId, $trip['id']]);

    $stmt = $pdo->prepare('DELETE FROM expense_participants WHERE expense_id = ?');
